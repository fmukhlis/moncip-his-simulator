...
export function SelectPanelDialog() {
  ...
  const { data: orderableServicesData, isFetching } = useQuery(
    getGetOrderableServicesActionOptions({ type: ["PANEL"], search: debouncedSearch, categories, trashed: false })
  )
  ...
  return (
    <Dialog>
      ...
      <DialogContent className="gap-3 sm:max-w-4xl">
        <DialogHeader>
          <DialogTitle>Add Preconfigured Panel</DialogTitle>
          <DialogDescription>Search and select panel to add to this order.</DialogDescription>
        </DialogHeader>
